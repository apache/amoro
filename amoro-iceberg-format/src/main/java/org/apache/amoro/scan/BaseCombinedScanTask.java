...
public class BaseCombinedScanTask implements CombinedScanTask {
  ...
  public BaseCombinedScanTask(NodeFileScanTask... tasks) {
    Preconditions.checkNotNull(tasks, "tasks cannot be null");
    this.tasks = tasks;
  }

  public BaseCombinedScanTask(List<NodeFileScanTask> tasks) {
    Preconditions.checkNotNull(tasks, "tasks cannot be null");
    this.tasks = tasks.stream().toArray(NodeFileScanTask[]::new);
  }
