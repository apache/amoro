...
public class MixedTableMaintainer implements TableMaintainer {
  ...
  private TableMaintainer changeMaintainer;

  private TableMaintainer baseMaintainer;
  ...
  public TableMaintainer getBaseMaintainer() {
  ...
  private class ChangeTableMaintainer extends IcebergTableMaintainer {
    ...
    protected long olderThanSnapshotNeedToExpire(TableRuntime tableRuntime) {
      ...
      return Longs.min(latestChangeFlinkCommitTime, changeTTLPoint);
    }
    ...
    private List<IcebergFileEntry> getExpiredDataFileEntries(long ttlPoint) {
      TableEntriesScan entriesScan = TableEntriesScan.builder(table)
      ...
      try (CloseableIterable<IcebergFileEntry> entries = entriesScan.entries()) {
        entries.forEach(entry -> {
          Snapshot snapshot = table.snapshot(entry.getSnapshotId());
    ...
    private void tryClearChangeFiles(List<DataFile> changeFiles) {
      ...
      try {
        for (int startIndex = 0; startIndex < changeFiles.size(); startIndex += DATA_FILE_LIST_SPLIT) {
          ...
          LOG.info("{} delete {} change files", table.name(), tableFiles.size());
          if (!tableFiles.isEmpty()) {
            DeleteFiles changeDelete = table.newDelete();
            changeFiles.forEach(changeDelete::deleteFile);
            changeDelete.commit();
          }
          LOG.info("{} change committed, delete {} files, complete {}/{}", table.name(),
              tableFiles.size(), end, changeFiles.size());
        }
      } catch (Throwable t) {
        LOG.error(table.name() + " failed to delete change files, ignore", t);
      }
    }
  }

  private class BaseTableMaintainer extends IcebergTableMaintainer {
    ...
    @Override
    protected long olderThanSnapshotNeedToExpire(TableRuntime tableRuntime) {
      return super.olderThanSnapshotNeedToExpire(tableRuntime);
    }
