 ...
 * submitted by user or system and handled by Amoro. The process could be a minor optimizing, a major
 * optimizing, an external optimizing, a snapshot refresh, a snapshot expire, a clean orphaned files
 * or a hive commit sync.
 * @param <T> the state type of the process
 */
public interface AmoroProcess<T extends ProcessState> {

  /**
   * Submit the process to Amoro. The process will be handled by Amoro after submitted.
   * If the process is already submitted, this method will do nothing.
   * For external optimizing, the process will be submitted to external resources like Yarn.
   */
  void submit();
  ...
  /**
   * Cancel and close this process, related resources will be released.
   * This method will block until getStatus() return CLOSED, but related resource could be released later.
   */
  void close();
