...
public interface DerbyOptimizeTasksMapper extends OptimizeTasksMapper {
  String TABLE_NAME = "optimize_task";

  @Select("select trace_id, optimize_type, catalog_name, db_name, table_name, partition," +
      " task_group, task_history_id, max_change_transaction_id, is_delete_pos_delete, is_adapt_hive," +
  ...
  @Results({
      ...
      @Result(property = "isAdaptHive", column = "is_adapt_hive"),
  ...
  @Insert("insert into " + TABLE_NAME + " (" +
          ...
          " status, pending_time, execute_time, prepared_time, report_time, commit_time," +
          " job_type, job_id, attempt_id, retry, fail_reason," +
          " fail_time, new_file_size, new_file_cnt, cost_time)" +
          " values(" +
          " #{optimizeTask.taskId.traceId, jdbcType=VARCHAR}," +
          " #{optimizeTask.taskId.type, jdbcType=VARCHAR}," +
          " #{optimizeTask.tableIdentifier.catalog, jdbcType=VARCHAR}," +
          " #{optimizeTask.tableIdentifier.database, jdbcType=VARCHAR}," +
          " #{optimizeTask.tableIdentifier.tableName, jdbcType=VARCHAR}," +
          " #{optimizeTask.partition, jdbcType=VARCHAR}," +
          " #{optimizeTask.taskGroup, jdbcType=VARCHAR}," +
          " #{optimizeTask.taskHistoryId, jdbcType=VARCHAR}," +
          " #{optimizeTask.maxChangeTransactionId}," +
          " #{optimizeTask.isDeletePosDelete}," +
          " #{optimizeTask.isAdaptHive}," +
          ...
          " #{optimizeTaskRuntime.status, jdbcType=VARCHAR}, " +
          ...
          "typeHandler=com.netease.arctic.ams.server.mybatis.Long2TsConvertor}," +
          " #{optimizeTaskRuntime.jobId.type, jdbcType=VARCHAR}," +
          " #{optimizeTaskRuntime.jobId.id, jdbcType=VARCHAR}," +
          " #{optimizeTaskRuntime.attemptId, jdbcType=VARCHAR}," +
          " #{optimizeTaskRuntime.retry}," +
          " #{optimizeTaskRuntime.failReason, jdbcType=VARCHAR}," +
