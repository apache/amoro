...
public interface DerbySnapInfoCacheMapper extends SnapInfoCacheMapper {
  ...
  @Insert("MERGE INTO " +
          ...
          TABLE_NAME +
          ".table_identifier= #{cacheFileInfo.tableIdentifier, typeHandler=" +
          "com.netease.arctic.ams.server.mybatis.TableIdentifier2StringConverter} and " +
          TABLE_NAME +
          ".inner_table = #{cacheFileInfo.innerTable} and " +
          TABLE_NAME +
          ".snapshot_id = #{cacheFileInfo.snapshotId} when matched THEN update set parent_snapshot_id = " +
          "#{cacheFileInfo.snapshotId}, action = #{cacheFileInfo.action}" +
          "WHEN NOT MATCHED THEN INSERT (table_identifier, snapshot_id, parent_snapshot_id, action," +
          " inner_table, commit_time)" +
          " values(#{cacheFileInfo.tableIdentifier," +
          "typeHandler=com.netease.arctic.ams.server.mybatis.TableIdentifier2StringConverter}," +
          " #{cacheFileInfo.snapshotId}, #{cacheFileInfo.parentSnapshotId}, #{cacheFileInfo.action}," +
  ...
  @Select("select snapshot_id,commit_time from " + TABLE_NAME + " where table_identifier = #{tableIdentifier," +
          " typeHandler=com.netease.arctic.ams.server.mybatis.TableIdentifier2StringConverter} and inner_table = " +
          "#{type} order by commit_time desc FETCH FIRST ROW ONLY")
  @Results({
