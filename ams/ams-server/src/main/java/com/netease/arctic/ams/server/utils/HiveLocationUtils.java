    ...
    if (TableTypeUtil.isHive(table)) {
      if (table.spec().isUnpartitioned()) {
        try {
          ...
          hiveLocations.add(FileUtil.getUriPath(hiveTable.getSd().getLocation()));
        } catch (Exception e) {
        ...
        try {
          ...
          for (Partition partition : partitions) {
            hiveLocations.add(FileUtil.getUriPath(partition.getSd().getLocation()));
          }
        } catch (Exception e) {
          LOG.error("{} get hive partitions error", table.id(), e);
          throw new IllegalStateException("get hive partitions error", e);
        }
      }
    }
