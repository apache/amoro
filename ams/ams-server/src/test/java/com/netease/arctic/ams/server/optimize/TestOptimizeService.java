...
import static com.netease.arctic.table.TableProperties.ENABLE_OPTIMIZE;
...
public class TestOptimizeService {
  ...
  public void testStopAndStartOptimize() throws NoSuchObjectException {
    ...
    table.refresh();
    Assert.assertEquals("false", table.properties().get(ENABLE_OPTIMIZE));
    service.startOptimize(com.netease.arctic.table.TableIdentifier.of(testOptimizeIdentifier));
    table.refresh();
    Assert.assertEquals("true", table.properties().get(ENABLE_OPTIMIZE));
  }
}
