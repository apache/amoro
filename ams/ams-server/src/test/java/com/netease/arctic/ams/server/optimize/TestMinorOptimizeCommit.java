...
public class TestMinorOptimizeCommit extends TestMinorOptimizePlan {
  ...
  public void testOnlyEqDeleteInChangeCommit() throws Exception {
    ...
    List<DataFile> changeEqDeletes = insertChangeDeleteFiles(testKeyedTable, transactionId);
    ...
    for (StructLike partitionDatum : partitionData) {
      Assert.assertEquals(transactionId, (long) newMaxTxId.get(partitionDatum));
    }
  }
  ...
  public void testNoPartitionOnlyEqDeleteInChangeCommit() throws Exception {
    long transactionId = 3;
    insertChangeDeleteFiles(testNoPartitionTable, transactionId);
    ...
    Assert.assertEquals(transactionId, (long) newMaxTxId.get(TablePropertyUtil.EMPTY_STRUCT));
  }
