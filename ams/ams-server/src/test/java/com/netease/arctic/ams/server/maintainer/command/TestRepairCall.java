...
public class TestRepairCall extends CallCommandTestBase {
  ...
  public void testRepairManifestListLoseThroughRollback() {
    ...
    Assert.assertFalse(getArcticTable().asKeyedTable()
        .changeTable()
        .currentSnapshot()
        .manifestListLocation()
        .equalsIgnoreCase(removeManifestList));
  }
  ...
  public void testRepairMetadataLoseThroughRollback() {
    ...
    testCatalog.loadTable(PK_TABLE_ID);
  }

  private String analyze(Context context) {
    return callFactory.generateAnalyzeCall(PK_TABLE_ID.toString()).call(context);
  }

  private String repair(Context context, RepairWay repairWay, Long option) {
    return callFactory.generateRepairCall(PK_TABLE_ID.toString(), repairWay, option).call(context);
  }
