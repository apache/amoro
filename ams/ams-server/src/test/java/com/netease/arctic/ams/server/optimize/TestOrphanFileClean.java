...
public class TestOrphanFileClean extends TestBaseOptimizeBase {
  ...
  public void orphanChangeDataFileInBaseClean() throws IOException {
    ...
    for (int i = 0; i < result.dataFiles().length; i++) {
      ...
      pathAll.add(FileUtil.getUriPath(dataFile.path().toString()));
      if (i == 0) {
        appendFiles.appendFile(dataFile).commit();
        fileInBaseStore.add(FileUtil.getUriPath(dataFile.path().toString()));
      } else {
        fileOnlyInChangeLocation.add(FileUtil.getUriPath(dataFile.path().toString()));
      }
    }
  ...
  private static class FakeArcticCatalog extends BaseArcticCatalog {
