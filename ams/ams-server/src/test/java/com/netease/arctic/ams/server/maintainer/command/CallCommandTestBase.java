...
public class CallCommandTestBase extends TableDataTestBase {
  ...
  public static CallFactory callFactory = new DefaultCallFactory(
      new MaintainerConfig(AMS.getUrl(), TEST_CATALOG_NAME),
      new CatalogManager(AMS.getUrl()),
      new PooledAmsClient(AMS.getUrl())
  );
  ...
  protected String removeFile() {
    ...
    String removeFile = Arrays.stream(fileScanTasksArray)
  ...
  protected int removeMetadata() {
    RepairTableOperation tableOperation = new RepairTableOperation(new CatalogManager(AMS.getUrl()),
        PK_TABLE_ID, ChangeLocationKind.INSTANT);
