...
case class RewriteUpdateMixedFormatTable(spark: SparkSession) extends Rule[LogicalPlan]
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan match {
    case u: UpdateTable if isMixedFormatRelation(u.table) =>
      val mixedTableRelation = asTableRelation(u.table)
      val upsertWrite = mixedTableRelation.table.asUpsertWrite
      val scanBuilder = upsertWrite.newUpsertScanBuilder(mixedTableRelation.options)
      if (u.condition.isEmpty) {
        val cond = Literal.TrueLiteral
        pushFilter(scanBuilder, cond, mixedTableRelation.output)
      } else {
        pushFilter(scanBuilder, u.condition.get, mixedTableRelation.output)
      }
      val upsertQuery =
        buildUpsertQuery(mixedTableRelation, upsertWrite, scanBuilder, u.assignments, u.condition)
      val query = upsertQuery
      var options: Map[String, String] = Map.empty
      options += (WriteMode.WRITE_MODE_KEY -> WriteMode.UPSERT.toString)
      val writeBuilder =
        MixedFormatSpark34Helper.newWriteBuilder(mixedTableRelation.table, query.schema, options)
      val write = writeBuilder.build()
      val projections = buildUpdateProjections(
        query,
        mixedTableRelation.output,
        MixedFormatExtensionUtils.isKeyedTable(mixedTableRelation))
      MixedFormatRowLevelWrite(mixedTableRelation, query, options, projections, Some(write))
