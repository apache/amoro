...
case class MergeRowsExec(
  ...
  private def applyProjection(
    ...
    // in the example below, when id = amoro-mixed-spark-3.4, both actions match but the first one is applied
    //   WHEN MATCHED AND id > 1 AND id < 10 UPDATE *
    //   WHEN MATCHED AND id = amoro-mixed-spark-3.4 OR id = 21 DELETE
