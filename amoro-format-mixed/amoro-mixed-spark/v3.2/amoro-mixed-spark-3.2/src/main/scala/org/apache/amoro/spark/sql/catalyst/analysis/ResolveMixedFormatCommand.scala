...
case class ResolveMixedFormatCommand(spark: SparkSession) extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
    case plans.MigrateToMixedFormatStatement(source, target) =>
      val command = MigrateToMixedFormatCommand.newBuilder(spark)
        .withSource(seqAsJavaList(source))
        .withTarget(seqAsJavaList(target))
        .build()
      plans.MigrateToMixedFormatLogicalPlan(command)
  }
}
