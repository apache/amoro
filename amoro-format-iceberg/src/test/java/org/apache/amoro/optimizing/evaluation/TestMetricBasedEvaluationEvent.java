...
public class TestMetricBasedEvaluationEvent extends TableTestBase {

  public TestMetricBasedEvaluationEvent(
  ...
  @Test
  public void test_evaluating_eventBasedTriggerEnabled() {
  ...
  public void test_evaluating_emptyTable() {
    ...
    MetricBasedEvaluationEvent.BasicTableStats stats =
        new MetricBasedEvaluationEvent.BasicTableStats(table);
    Assert.assertEquals(0, stats.dataFileCnt);
    Assert.assertEquals(0, stats.fileSize);
    Assert.assertEquals(0, stats.deleteFileCnt);

    // Verify the empty table evaluation
    Assert.assertTrue(config.isEventBasedTriggerEnabled());
    Assert.assertFalse(MetricBasedEvaluationEvent.isReachFallbackInterval(config, 0L));
    Assert.assertFalse(MetricBasedEvaluationEvent.isEvaluatingNecessary(config, table, 0L));
  }
  ...
  public void test_evaluating_nonEmptyTable() throws IOException {
    ...
    Assert.assertTrue(stats.fileSize > 0);
    Assert.assertEquals(0, stats.deleteFileCnt);

    // 1. Test for event-based trigger disabled.
    config.setEvaluationFallbackInterval(-1);
    Assert.assertFalse(config.isEventBasedTriggerEnabled());

    // 2. Test for event-based trigger enabled.
    // Temporarily set self-optimizing.evaluation.fallback-interval to Long.MAX_VALUE to prevent
    // triggering due to reaching the fallback interval.
    config.setEvaluationFallbackInterval(Long.MAX_VALUE);
    Assert.assertTrue(config.isEventBasedTriggerEnabled());
    ...
    Assert.assertTrue(MetricBasedEvaluationEvent.isEvaluatingNecessary(config, table, 0L));
  ...
  public void test_evaluating_pendingInput_nonEmptyTable() throws IOException {
    initData();
    // Set event-based trigger enabled and fallback interval not reached.
    OptimizingConfig config =
        getDefaultOptimizingConfig().setEvaluationFallbackInterval(Long.MAX_VALUE);
    MixedTable table = getMixedTable();
  ...
  public void testBasicStatsAccept() {
    MetricBasedEvaluationEvent.BasicTableStats stats =
        new MetricBasedEvaluationEvent.BasicTableStats();
    ...
    Assert.assertEquals(1024, stats.fileSize);
    ...
    Assert.assertEquals(3072, stats.fileSize); // 1024 + 2048
  }
  ...
  public void testBasicTableStatsAcceptWithMissingProperties() {
    ...
    Assert.assertEquals(0, stats.fileSize); // default value
  }
  ...
  @Test
  public void testBasicTableStatsAcceptWithMemorySizeParsing() {
    MetricBasedEvaluationEvent.BasicTableStats stats =
        new MetricBasedEvaluationEvent.BasicTableStats();
    // Test various memory size formats
    Map<String, String> summary = new HashMap<>();
    summary.put("total-delete-files", "2");
    summary.put("total-data-files", "5");
    summary.put("total-files-size", "1KB"); // Should parse to 1024 bytes
    stats.accept(summary);
    Assert.assertEquals(2, stats.deleteFileCnt);
    Assert.assertEquals(5, stats.dataFileCnt);
    Assert.assertEquals(1024, stats.fileSize); // 1KB = 1024 bytes
  }
