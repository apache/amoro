...
/**
 * Copy from iceberg {@link org.apache.iceberg.parquet.TypeToMessageType} for follow reason: 1. Make
 * timestamp to int96. 2. remove {@link AvroSchemaUtil#makeCompatibleName(String)} of name.
 */
public class AdaptHiveTypeToMessageType {
  ...
  public GroupType list(
      org.apache.iceberg.types.Types.ListType list,
      Type.Repetition repetition,
      int id,
      String name) {
    org.apache.iceberg.types.Types.NestedField elementField = list.fields().get(0);
    return Types.list(repetition).element(field(elementField)).id(id).named(name);
  }

  public GroupType map(
      org.apache.iceberg.types.Types.MapType map, Type.Repetition repetition, int id, String name) {
    ...
    return Types.map(repetition).key(field(keyField)).value(field(valueField)).id(id).named(name);
  }

  public Type primitive(
      org.apache.iceberg.types.Type.PrimitiveType primitive,
      Type.Repetition repetition,
      int id,
      String originalName) {
    ...
    switch (primitive.typeId()) {
      ...
      case FIXED:
        org.apache.iceberg.types.Types.FixedType fixed =
            (org.apache.iceberg.types.Types.FixedType) primitive;
      ...
      case DECIMAL:
        org.apache.iceberg.types.Types.DecimalType decimal =
            (org.apache.iceberg.types.Types.DecimalType) primitive;
