...
| `amoro-format-paimon/src/main/java/org/apache/amoro/formats/paimon/optimizing/primary/PaimonPrimaryKeyOptions.java` | enabled 默认 false；不再提供 bucket 合并参数，长期 compaction 固定按一个 partition-bucket unit 一个 task 下发；`partition-idle-time` 支持 Paimon duration 和 ISO-8601；major 阈值有合法性校验。 |
