...
| auto-create-tags.interval | PT1M | Interval for creating tags. |
| auto-create-tags.thread-count | 3 | The number of threads used for creating tags. |
| blocker.timeout | PT1M | Session timeout. Default unit is milliseconds if not specified. |
| catalog-meta-cache.expiration-interval | PT1M | TTL for catalog metadata. |
...
| clean-orphan-files.interval | PT24H | Interval for cleaning orphan files. |
...
| data-expiration.interval | PT24H | Execute interval for data expiration |
...
| ha.connection-timeout | PT5M | The Zookeeper connection timeout in milliseconds. |
| ha.enabled | false | Whether to enable high availability mode. |
| ha.session-timeout | PT30S | The Zookeeper session timeout in milliseconds. |
...
| http-server.session-timeout | PT168H | Timeout for http session. |
| optimizer.heart-beat-timeout | PT1M | Timeout duration for Optimizer heartbeat. |
| optimizer.max-planning-parallelism | 1 | Max planning parallelism in one optimizer group. |
| optimizer.polling-timeout | PT3S | Optimizer polling task timeout. |
| optimizer.task-ack-timeout | PT30S | Timeout duration for task acknowledgment. |
| optimizer.task-execute-timeout | PT1H | Timeout duration for task execution, default to 1 hour. |
| overview-cache.max-size | 3360 | Max size of overview cache. |
| overview-cache.refresh-interval | PT3M | Interval for refreshing overview cache. |
| refresh-external-catalogs.interval | PT3M | Interval to refresh the external catalog. |
...
| refresh-tables.interval | PT1M | Interval for refreshing table metadata. |
...
| self-optimizing.refresh-group-interval | PT30S | Optimizer group refresh interval. |
...
| terminal.session.timeout | PT30M | Session timeout. Default unit is milliseconds if not specified (** Note: default units are minutes when version &lt; 0.8). |
