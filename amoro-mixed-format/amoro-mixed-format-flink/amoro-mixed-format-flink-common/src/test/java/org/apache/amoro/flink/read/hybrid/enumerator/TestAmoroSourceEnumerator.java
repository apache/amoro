...
public class TestAmoroSourceEnumerator extends FlinkTestBase {

  public TestAmoroSourceEnumerator() {
    super(
  ...
  public void testReadersNumGreaterThanSplits() throws Exception {
    ...
    AmoroScanContext scanContext =
        AmoroScanContext.arcticBuilder()
            .streaming(true)
            .scanStartupMode(SCAN_STARTUP_MODE_EARLIEST)
            .build();

    List<AmoroSplit> splitList =
    ...
    AmoroSourceEnumerator enumerator =
        new AmoroSourceEnumerator(
    ...
    Collection<AmoroSplitState> pendingSplitsEmpty = enumerator.snapshotState(1).pendingSplits();
  ...
  private void assertSnapshot(ShuffleSplitAssigner assigner, int splitCount) {
    Collection<AmoroSplitState> stateBeforeGet = assigner.state();
  ...
  protected static class TestingSplitEnumeratorContext
      implements SplitEnumeratorContext<AmoroSplit> {
    private final int parallelism;

    private final HashMap<Integer, SplitAssignmentState<AmoroSplit>> splitAssignments =
        new HashMap<>();
    ...
    public Map<Integer, SplitAssignmentState<AmoroSplit>> getSplitAssignments() {
    ...
    @Override
    public void assignSplits(SplitsAssignment<AmoroSplit> newSplitAssignments) {
      for (final Map.Entry<Integer, List<AmoroSplit>> entry :
          newSplitAssignments.assignment().entrySet()) {
        final SplitAssignmentState<AmoroSplit> assignment =
    ...
    @Override
    public void assignSplit(AmoroSplit split, int subtask) {
