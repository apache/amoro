...
public class TestAmoroSplitSerializer extends TestRowDataReaderFunction {
  ...
  private void assertSerializedSplitEquals(List<AmoroSplit> expected) {
    AmoroSplitSerializer serializer = new AmoroSplitSerializer();
    List<byte[]> contents =
        expected.stream()
            .map(
                split -> {
    ...
    Assert.assertArrayEquals(
        expected.toArray(new AmoroSplit[0]),
        contents.stream()
            .map(
                data -> {
                  if (data.length == 0) {
  ...
  public void testNullableSplit() throws IOException {
    ...
    AmoroSplit actual = serializer.deserialize(1, ser);
