...
/** Serializer that serializes and deserializes arctic enumerator {@link AmoroSourceEnumState}. */
public class AmoroSourceEnumStateSerializer
    implements SimpleVersionedSerializer<AmoroSourceEnumState> {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(AmoroSourceEnumStateSerializer.class);
  private static final int VERSION = 1;
  private final AmoroSplitSerializer splitSerializer = AmoroSplitSerializer.INSTANCE;
  private final AmoroEnumeratorOffsetSerializer offsetSerializer =
      AmoroEnumeratorOffsetSerializer.INSTANCE;
  ...
  @Override
  public byte[] serialize(AmoroSourceEnumState arcticSourceEnumState) throws IOException {
    return serializeV1(arcticSourceEnumState);
  }

  private byte[] serializeV1(AmoroSourceEnumState enumState) throws IOException {
    ...
    for (AmoroSplitState splitState : enumState.pendingSplits()) {
  ...
  @Override
  public AmoroSourceEnumState deserialize(int version, byte[] serialized) throws IOException {
    switch (version) {
      case 1:
  ...
  private AmoroSourceEnumState deserializeV1(byte[] serialized) throws IOException {
    DataInputDeserializer in = new DataInputDeserializer(serialized);

    AmoroEnumeratorOffset enumeratorOffset = null;
    if (in.readBoolean()) {
    ...
    Collection<AmoroSplitState> pendingSplits = Lists.newArrayListWithCapacity(splitCount);
    for (int i = 0; i < splitCount; ++i) {
      byte[] splitBytes = new byte[in.readInt()];
      in.read(splitBytes);
      AmoroSplit split = splitSerializer.deserialize(splitSerializerVersion, splitBytes);
      pendingSplits.add(new AmoroSplitState(split));
    }
    ...
    return new AmoroSourceEnumState(
