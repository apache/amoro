...
public class AmoroScanContext extends ScanContext implements Serializable {
  ...
  protected AmoroScanContext(Builder builder) {
    super(
  ...
  public static class Builder {
    ...
    public Builder fromProperties(Map<String, String> properties) {
      ...
      return this.useSnapshotId(config.get(SNAPSHOT_ID))
          ...
          .scanStartupMode(properties.get(ArcticValidator.SCAN_STARTUP_MODE.key()))
    ...
    public AmoroScanContext build() {
      scanStartupMode = scanStartupMode == null ? null : scanStartupMode.toLowerCase();
      Preconditions.checkArgument(
          Objects.isNull(scanStartupMode)
              || Objects.equals(scanStartupMode, ArcticValidator.SCAN_STARTUP_MODE_EARLIEST)
              || Objects.equals(scanStartupMode, ArcticValidator.SCAN_STARTUP_MODE_LATEST),
          String.format(
              "only support %s, %s when %s is %s",
              ArcticValidator.SCAN_STARTUP_MODE_EARLIEST,
              ArcticValidator.SCAN_STARTUP_MODE_LATEST,
              ArcticValidator.ARCTIC_READ_MODE,
              ArcticValidator.ARCTIC_READ_FILE));
      Preconditions.checkArgument(
          !(isStreaming && batchMode),
          String.format(
              "only support %s = false when execution.runtime-mode is batch", STREAMING.key()));
      return new AmoroScanContext(this);
    }
  }
}
