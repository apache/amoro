...
public class FlinkSink {
  ...
  public static class Builder {
    ...
    private Builder forRowData(DataStream<RowData> newRowDataInput) {
      this.inputCreator =
          new Function<String, DataStream<RowData>>() {
            @Override
            public DataStream<RowData> apply(String ignored) {
              return newRowDataInput;
            }
          };
      return this;
    }
      ...
      this.inputCreator =
          new Function<String, DataStream<RowData>>() {
            ...
            public DataStream<RowData> apply(String newUidPrefix) {
              ...
              // of map operator same as its input to keep map operator chaining its input, and
              // avoid
    ...
    public DataStreamSink<Void> append() {
      if (tableMetaStore != null) {
        return tableMetaStore.doAs(this::chainIcebergOperators);
      }
    ...
    private SingleOutputStreamOperator<Void> appendCommitter(
        SingleOutputStreamOperator<WriteResult> writerStream) {
      OneInputStreamOperator<WriteResult, Void> filesCommitter = createFileCommitter();
      SingleOutputStreamOperator<Void> committerStream =
          writerStream
    ...
    private OneInputStreamOperator<WriteResult, Void> createFileCommitter() {
      IcebergFilesCommitter filesCommitter =
          new IcebergFilesCommitter(
              ...
              table.spec());
      if (tableMetaStore == null) {
        return filesCommitter;
      }

      return (OneInputStreamOperator<WriteResult, Void>)
          ProxyUtil.getProxy(filesCommitter, tableMetaStore);
    }
    ...
    private SingleOutputStreamOperator<WriteResult> appendWriter(
      ...
      if (tableRefreshInterval != null) {
        ...
        tableSupplier =
            new SerializableSupplier<Table>() {
              @Override
              public Table get() {
      ...
      OneInputStreamOperator<RowData, WriteResult> streamWriterOperator = streamWriter;

      if (tableMetaStore != null) {
        streamWriterOperator =
            (OneInputStreamOperator<RowData, WriteResult>)
                ProxyUtil.getProxy(streamWriter, tableMetaStore);
      }
