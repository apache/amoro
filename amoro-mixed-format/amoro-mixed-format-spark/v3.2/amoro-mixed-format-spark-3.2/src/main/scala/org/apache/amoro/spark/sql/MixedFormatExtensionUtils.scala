...
object MixedFormatExtensionUtils {
  ...
  implicit class MixedFormatTableHelper(table: Table) {
    def asMixedSparkTable: MixedSparkTable = {
      table match {
        case table: MixedSparkTable => table
  ...
  def isUnkeyedTableRelation(plan: LogicalPlan): Boolean = {
    ...
    plan.collectLeaves().exists {
      case p: DataSourceV2Relation => isIcebergTable(p)
