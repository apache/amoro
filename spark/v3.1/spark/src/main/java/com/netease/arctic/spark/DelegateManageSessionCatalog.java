 ...
 */
public class DelegateManageSessionCatalog <T extends TableCatalog & SupportsNamespaces>
    implements StagingTableCatalog, SupportsNamespaces, CatalogExtension {

  public final String DELEGATES = "delegates";

  private final Set<String> INNER_OPTIONS = ImmutableSet.of(DELEGATES);

  private CaseInsensitiveStringMap options;

  private CatalogExtension delegateCatalog;

  @Override
  public void initialize(String name, CaseInsensitiveStringMap options) {
    Preconditions.checkArgument("spark_catalog".equalsIgnoreCase(name),
        DelegateManageSessionCatalog.class.getName() + " can only be used for spark_catalog");
    Preconditions.checkArgument(options.containsKey(DELEGATES),
        "lack require parameter " + DELEGATES);
  ...
  public void setDelegateCatalog(CatalogPlugin delegate) {
    List<CatalogExtension> delegates = getCatalogs(this.options);
    Preconditions.checkArgument(delegates.size() > 0,
        "delegates can not be empty");

    Iterator<CatalogExtension> iterator = delegates.iterator();
    CatalogExtension delegateCatalog = iterator.next();
    CatalogExtension catalog = delegateCatalog;

    while (iterator.hasNext()) {
      CatalogExtension nextCatalog = iterator.next();
  ...
  private List<CatalogExtension> getCatalogs(CaseInsensitiveStringMap options){
    Map<String, Map<String, String>> catalogOptions = Maps.newHashMap();
    Map<String, String> catalogClassName = Maps.newHashMap();
    List<String> catalogs = Lists.newArrayList(options.get(DELEGATES).split(","));
    ...
    for (String key: options.keySet()){
      if (INNER_OPTIONS.contains(key)){
        ...
        Preconditions.checkArgument(catalogOptions.containsKey(catalog),
  ...
  private CatalogExtension loadCatalog(String catalogName, String className, Map<String, String> options) {
    ...
    try {
      ...
      throw new IllegalStateException("Cannot find a no-arg constructor for delegate catalog plugin class for catalog "
          + catalogName + ": " + className, e);
    } catch (InvocationTargetException e) {
      throw new IllegalStateException("Failed during call to no-arg constructor for delegate catalog plugin class for catalog "
          + catalogName + ": " + className, e);
    } catch (InstantiationException | IllegalAccessException e) {
      throw new IllegalStateException("Failed to call public no-arg constructor for delegate catalog plugin class for catalog "
          + catalogName + ": " + className, e);
    }
  }
