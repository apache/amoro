...
public abstract class MixedSessionCatalogBase<T extends TableCatalog & SupportsNamespaces>
  ...
  private MixedSparkCatalogBase mixedFormatSparkCatalog = null;
  ...
  public void initialize(String name, CaseInsensitiveStringMap options) {
    ...
    try {
      this.mixedFormatSparkCatalog = buildMixedFormatSparkCatalog(name, options);
    } catch (Exception e) {
      this.mixedFormatSparkCatalog = null;
    }
  }
   ...
   */
  protected abstract MixedSparkCatalogBase buildMixedFormatSparkCatalog(
      String name, CaseInsensitiveStringMap options);

  private MixedSparkCatalogBase getMixedFormatSparkCatalog() {
    if (mixedFormatSparkCatalog == null) {
      this.mixedFormatSparkCatalog = buildMixedFormatSparkCatalog(this.catalogName, this.options);
    }
    return this.mixedFormatSparkCatalog;
  }
  ...
  public Table loadTable(Identifier ident) throws NoSuchTableException {
    ...
    if (MixedFormatSparkUtil.isMixedFormatTable(table)) {
      return getMixedFormatSparkCatalog().loadTable(ident);
    }
    return table;
  }

  private boolean isMixedFormatProvier(String provider) {
  ...
  public Table createTable(
    ...
    if (isMixedFormatProvier(provider)) {
      return getMixedFormatSparkCatalog().createTable(ident, schema, partitions, properties);
    } else {
  ...
  public Table alterTable(Identifier ident, TableChange... changes) throws NoSuchTableException {
    Table table = getSessionCatalog().loadTable(ident);
    if (MixedFormatSparkUtil.isMixedFormatTable(table)) {
      return getMixedFormatSparkCatalog().alterTable(ident, changes);
    } else {
  ...
  public boolean dropTable(Identifier ident) {
    ...
    try {
      Table table = getSessionCatalog().loadTable(ident);
      if (MixedFormatSparkUtil.isMixedFormatTable(table)) {
        return getMixedFormatSparkCatalog().dropTable(ident)
  ...
  public void renameTable(Identifier from, Identifier to)
    ...
    if (MixedFormatSparkUtil.isMixedFormatTable(table)) {
      getMixedFormatSparkCatalog().renameTable(from, to);
    } else {
