...
import com.netease.arctic.spark.optimize.{Expression, Literal, NamedReference, Transform}
...
private[sql] object LogicalExpressions {
  ...
  def parseReference(name: String): NamedReference =
    FieldReference(CatalystSqlParser.parseFunctionIdentifier(name).toString())
