...
public interface OptimizingProcessMapper {
  ...
  @Select(
      "SELECT COUNT(1) FROM task_runtime WHERE table_id = #{tableId} AND process_id = #{processId}"
          + " AND status IN ('PLANNED', 'SCHEDULED', 'ACKED')")
  int countUnfinishedTasks(@Param("tableId") long tableId, @Param("processId") long processId);
