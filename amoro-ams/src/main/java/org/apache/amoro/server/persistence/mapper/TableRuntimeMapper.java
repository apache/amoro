...
public interface TableRuntimeMapper {
  ...
  @Select(
      "<script>"
          ...
          + "<bind name=\"isSQLite\" value=\"_databaseId == 'sqlite'\" />"
          ...
          + "<if test='fuzzyDbName != null and (isPostgreSQL or isSQLite)'> AND i.db_name like '%' || #{fuzzyDbName, jdbcType=VARCHAR} || '%' </if>"
          + "<if test='fuzzyTableName != null and isMySQL'> AND i.table_name like CONCAT('%', #{fuzzyTableName, jdbcType=VARCHAR}, '%') </if>"
          + "<if test='fuzzyTableName != null and (isPostgreSQL or isSQLite)'> AND i.table_name like '%' || #{fuzzyTableName, jdbcType=VARCHAR} || '%' </if>"
