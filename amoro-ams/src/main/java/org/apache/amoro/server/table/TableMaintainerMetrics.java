 ...
 *   <li>Operation duration is recorded exclusively via {@link
 *       MaintainerMetrics#recordOperationSuccess} / {@link
 *       MaintainerMetrics#recordOperationFailure} to ensure a single source of truth. The specific
 *       per-operation duration gauges (e.g., {@code table_snapshots_expiration_duration_millis})
 *       are updated from these callbacks based on the {@link
 *       org.apache.amoro.maintainer.MaintainerOperationType}.
 *   <li>This class does not extend {@link AbstractTableMetrics} because it requires a {@code
 *       table_format} tag on every metric. Future format-specific subclasses (e.g., for Paimon
 *       partition expiration) can extend this class to add format-unique indicators while reusing
 *       the shared registration infrastructure.
 * </ul>
 */
public class TableMaintainerMetrics implements MaintainerMetrics {
  ...
  public static final MetricDefine TABLE_ORPHAN_METADATA_FILES_CLEANED_COUNT =
      // Keep historical metric name for backward compatibility.
  ...
  public void recordOperationSuccess(MaintainerOperationType operationType, long durationMillis) {
    ...
    updateSpecificDuration(operationType, durationMillis);
  }
  ...
  private void updateSpecificDuration(MaintainerOperationType operationType, long durationMillis) {
    switch (operationType) {
      case ORPHAN_FILES_CLEANING:
        orphanFilesCleaningDuration.setValue(durationMillis);
        break;
      case DANGLING_DELETE_FILES_CLEANING:
        danglingDeleteFilesCleaningDuration.setValue(durationMillis);
        break;
      case SNAPSHOT_EXPIRATION:
        snapshotsExpirationDuration.setValue(durationMillis);
        break;
      case DATA_EXPIRATION:
  ...
  /** Gauge implementation for recording operation duration */
