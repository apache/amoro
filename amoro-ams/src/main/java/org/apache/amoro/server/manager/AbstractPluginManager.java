...
public abstract class AbstractPluginManager<T extends ActivePlugin> implements PluginManager<T> {
  ...
  public AbstractPluginManager(String pluginCategory) {
    this(pluginCategory, new Configurations(), null);
  }

  @SuppressWarnings("unchecked")
  public AbstractPluginManager(String pluginCategory, ConfigurationManager configurationManager) {
    this(pluginCategory, new Configurations(), configurationManager);
  }
    ...
    installedPlugins.computeIfAbsent(
        ...
        name -> {
          ...
          if (configurationManager != null) {
            configurations =
                new DynamicConfigurations(
                    Configurations.fromMap(augmentedProperties),
                    configurationManager,
                    pluginCategory(),
                    pluginConfig.getName());
          } else {
            configurations = Configurations.fromMap(augmentedProperties);
          }
          plugin.open(configurations);
          exists.set(false);
          return plugin;
        });
    if (exists.get()) {
