...
/** Local execution engine that runs {@link LocalProcess} instances in shared AMS thread pools. */
public class LocalExecutionEngine implements ExecuteEngine {
  ...
  private final Map<String, ProcessStatus> terminalStatuses = new ConcurrentHashMap<>();
  ...
  public ProcessStatus getStatus(String processIdentifier) {
    ...
    if (future == null) {
      return terminalStatuses.getOrDefault(processIdentifier, ProcessStatus.KILLED);
    }

    if (future.isCancelled()) {
      instances.remove(processIdentifier);
      terminalStatuses.put(processIdentifier, ProcessStatus.CANCELED);
    ...
    if (future.isDone()) {
      ...
      try {
        future.get();
        terminalStatuses.put(processIdentifier, ProcessStatus.SUCCESS);
        return ProcessStatus.SUCCESS;
      } catch (Exception e) {
        terminalStatuses.put(processIdentifier, ProcessStatus.FAILED);
    ...
    if (!(tableProcess instanceof LocalProcess)) {
      throw new IllegalArgumentException(
          "LocalExecutionEngine only supports LocalProcess, but got: "
              + tableProcess.getClass().getName());
    }

    LocalProcess localProcess = (LocalProcess) tableProcess;
    String identifier = UUID.randomUUID().toString();
    Future<?> future =
        getOrCreatePool(localProcess.tag())
            .submit(
                () -> {
                  localProcess.run();
                  return null;
  ...
  public void open(Map<String, String> properties) {
    int defaultSize =
        parseInt(properties == null ? null : properties.get("default.thread-count"), 10);
    pools.put(DEFAULT_POOL, newFixedPool(DEFAULT_POOL, defaultSize));

    int syncPoolSize =
        parseInt(
            properties == null ? null : properties.get("table-meta-sync.thread-count"),
            defaultSize);
    pools.put(TABLE_META_SYNC_POOL, newFixedPool(TABLE_META_SYNC_POOL, Math.max(syncPoolSize, 1)));
  }
  ...
  private ThreadPoolExecutor getOrCreatePool(String tag) {
    String poolTag = (tag == null || tag.isEmpty()) ? DEFAULT_POOL : tag;
    return pools.computeIfAbsent(poolTag, t -> newFixedPool(t, 10));
  }
  ...
  private ThreadPoolExecutor newFixedPool(String tag, int size) {
    return new ThreadPoolExecutor(
        ...
        new ThreadFactoryBuilder().setDaemon(true).setNameFormat("local-" + tag + "-%d").build());
  }
