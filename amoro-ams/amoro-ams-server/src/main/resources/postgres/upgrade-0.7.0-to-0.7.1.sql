...
SELECT  `table_id`,`catalog_name`, `db_name`, `table_name`, `current_snapshot_id`,`current_change_snapshotId`, `last_optimized_snapshotId`,
        `last_optimized_change_snapshotId`, `last_major_optimizing_time`, `last_minor_optimizing_time`, `last_full_optimizing_time`,
        CASE
            WHEN `optimizing_status` = 'IDLE' THEN 6
            WHEN `optimizing_status` = 'PENDING' THEN 5
            WHEN `optimizing_status` = 'PLANNING' THEN 4
            WHEN `optimizing_status` = 'COMMITTING' THEN 3
            WHEN `optimizing_status` = 'MINOR_OPTIMIZING' THEN 2
            WHEN `optimizing_status` = 'MAJOR_OPTIMIZING' THEN 1
            WHEN `optimizing_status` = 'FULL_OPTIMIZING' THEN 0
            END,
