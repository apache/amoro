...
/**
 * This class copies part of the code of IcebergStatistics to adapt to Arctic
 */
@Immutable
final class KeyedTableStatistics {
  ...
  public static class Builder {
    ...
    public void acceptDataFile(DataFile dataFile, PartitionSpec partitionSpec, DataFileType dataFileType) {
      ...
      for (Types.NestedField column : partitionSpec.schema().columns()) {
        ...
        if (identityPartitionFieldIds.contains(id)) {
          verify(partitionValues.containsKey(id),
              "Unable to find value for partition column with field id " + id);
          ...
          Object lowerBound = convertIcebergValueToTrino(column.type(),
              Conversions.fromByteBuffer(column.type(), Optional.ofNullable(
                  dataFile.lowerBounds()).map(a -> a.get(id)).orElse(null)));
          Object upperBound = convertIcebergValueToTrino(column.type(),
              Conversions.fromByteBuffer(column.type(), Optional.ofNullable(
                  dataFile.upperBounds()).map(a -> a.get(id)).orElse(null)));
          Optional<Long> nullCount = Optional.ofNullable(
              dataFile.nullValueCounts()).map(nullCounts -> nullCounts.get(id));
          updateMinMaxStats(
              id,
              trinoType,
              lowerBound,
    ...
    public KeyedTableStatistics build() {
      ...
      return new KeyedTableStatistics(
          recordCount,
          fileCount,
          size,
    ...
    private void updateNullCountStats(int id, Optional<Long> nullCount) {
      ...
      nullCounts.merge(id, nullCount, (existingCount, newCount) ->
          existingCount.isPresent() && newCount.isPresent() ? Optional.of(
              existingCount.get() + newCount.get()) : Optional.empty());
    }
    ...
    private void updateMinMaxStats(
      ...
      if (type.isOrderable() && (nullCount.isEmpty() || nullCount.get() != recordCount)) {
        // Capture the initial bounds during construction so there are always valid min/max values to compare to.
        // This does make the first call to
