...
public class ArcticTimer {
  ...
  private ArcticTimer(boolean shouldStart) {
    if (shouldStart) {
      startTimer();
    }
  }
  ...
  public long endTimer() {
    if (timeInfoDeque.isEmpty()) {
      throw new ArcticException("Timer was not started");
    }
    return timeInfoDeque.pop().stop();
  }

  public static ArcticTimer start() {
    return new ArcticTimer(true);
  }

  public static ArcticTimer create() {
    return new ArcticTimer(false);
  }
}
