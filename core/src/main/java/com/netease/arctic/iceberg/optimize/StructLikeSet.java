...
public class StructLikeSet extends AbstractSet<StructLike> implements Set<StructLike> {
  ...
  private StructLikeWrapperFactory structLikeWrapperFactory;
  ...
  public boolean add(StructLike struct) {
    return wrapperSet.add(structLikeWrapperFactory.create().set(struct));
  }
  ...
  public boolean addAll(Collection<? extends StructLike> structs) {
    if (structs != null) {
      return Iterables.addAll(wrapperSet,
          Iterables.transform(structs, struct -> structLikeWrapperFactory.create().set(struct)));
    }
    return false;
  }
