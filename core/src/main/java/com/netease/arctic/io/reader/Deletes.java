...
public class Deletes {
  ...
  public static <T> CloseableIterable<T> filter(
      CloseableIterable<T> rows, Function<T, Long> rowToPosition, Set<Long> deleteSet) {
  ...
  public static Set<Long> toPositionSet(
      CharSequence dataLocation, CloseableIterable<? extends StructLike> deleteFile) {
    return toPositionSet(dataLocation, ImmutableList.of(deleteFile));
  }

  public static <T extends StructLike> Set<Long> toPositionSet(
  ...
  private static class DataFileFilter<T extends StructLike> extends Filter<T> {
    ...
    protected boolean shouldKeep(T posDelete) {
      return CHARSEQ_COMPARATOR.compare(
              dataLocation, (CharSequence) FILENAME_ACCESSOR.get(posDelete))
          == 0;
    }
  }
