...
public class LogReadHelper implements Serializable {
  ...
  /**
   * this map is relate upstream id and topic partitions to the retracting epicNo, also describe
   * whether retracting right now. Key : String format, upstream job id + "_" + topic partition,
   * generated by {@link #combineUpstreamIdAndPartition)} method. Value : {@link
   * EpicRetractingOffset}.
   */
  ...
  public void initializeState(FunctionInitializationContext context, RuntimeContext runtimeContext)
      throws Exception {
    OperatorStateStore stateStore = context.getOperatorStateStore();
    retractingRestoreState =
        stateStore.getUnionListState(
            new ListStateDescriptor<>(
                STATE_EPIC_RETRACTING, createStateSerializer(runtimeContext.getExecutionConfig())));

    epicRestoreState =
        stateStore.getUnionListState(
            new ListStateDescriptor<>(
                STATE_EPIC, createEpicStateSerializer(runtimeContext.getExecutionConfig())));

    Map<String, LogEpicStateHandler.EpicPartitionOffsets> upstreamEpicOffsets =
        new ConcurrentHashMap<>();
    retractingState = Maps.newTreeMap();
    if (context.isRestored()) {
      for (Tuple2<String, EpicRetractingOffset> upstreamIdPartitionRetractingOffset :
          retractingRestoreState.get()) {
        retractingState.put(
            upstreamIdPartitionRetractingOffset.f0, upstreamIdPartitionRetractingOffset.f1);
      }
      for (Tuple2<String, LogEpicStateHandler.EpicPartitionOffsets> epicOffset :
          epicRestoreState.get()) {
  ...
  public void snapshotState() throws Exception {
    ...
    for (Map.Entry<String, EpicRetractingOffset> retractingOffsetEntry :
        retractingState.entrySet()) {
    ...
    for (Map.Entry<String, LogEpicStateHandler.EpicPartitionOffsets> epicOffsets :
      ...
      epicRestoreState.add(Tuple2.of(epicOffsets.getKey(), epicOffsets.getValue()));
    }
  }
   ...
   * check this job
   *
   * <p>upstream job id whether retracting right now.
   *
  ...
  /** starting retract */
  public void markEpicPartitionRetracting(
      String upstreamId, long epicNo, int partition, Long offset) {
    String key = combineUpstreamIdAndPartition(upstreamId, partition);
  ...
  public void updateEpicStartOffsetIfEmpty(
      String upstreamId, long epicNo, int partition, long startOffset) {
    epicStateHandler.registerEpicPartitionStartOffset(upstreamId, epicNo, partition, startOffset);
  }

  public void updateRetractingEpicOffset(
      String upstreamId, long epicNo, int partition, long offset) {
    String key = combineUpstreamIdAndPartition(upstreamId, partition);
    ...
    if (retractingOffset != null) {
      ...
      throw new FlinkRuntimeException(
          "There may be a bug, because can't find retracting epicNo offset.");
    }
  }
  ...
  public long queryRetractingEpicNo(String upstreamId, int partition) {
    ...
    if (!retractingState.containsKey(key)) {
      throw new FlinkRuntimeException(
          "Can't find epicNo, upstreamId=" + upstreamId + ", partition=" + partition);
    }
  ...
  public Optional<Long> getEpicOffset(String upstreamId, long epicNo, int partition) {
    ...
    return epicNoFlipInfoOptional.map(
        epicPartitionOffsets -> {
  ...
  private TupleSerializer<Tuple2<String, LogEpicStateHandler.EpicPartitionOffsets>>
      createEpicStateSerializer(ExecutionConfig executionConfig) {
  ...
  public boolean getCleanBufferAction(String upstreamId, int partition) {
    return upstreamIdPartitionCleanBufferMap.getOrDefault(
        combineUpstreamIdAndPartition(upstreamId, partition), false);
  }
  ...
  public void cleanBufferAction(String upstreamId, int partition, boolean enableOrDisable) {
    upstreamIdPartitionCleanBufferMap.put(
        combineUpstreamIdAndPartition(upstreamId, partition), enableOrDisable);
  }

  /**
   * The epic and offset information is recorded while the retracting state is in progress and the
   * offset content is updated in real time.
   */
