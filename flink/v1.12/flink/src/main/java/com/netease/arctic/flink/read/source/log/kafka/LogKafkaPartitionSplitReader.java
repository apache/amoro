 ...
 * #readNormal()} - convert data to {@link LogRecordKafkaWithRetractInfo} in {@link
 ...
 * {@link LogKafkaPartitionSplitState#updateState(LogRecordKafkaWithRetractInfo)} 4. initialize
 * state from state {@link LogSourceHelper#initializedState}
 */
    ...
    for (Map.Entry<TopicPartition, List<ConsumerRecord<byte[], byte[]>>> entry :
      ...
      for (int i = startIndex; i >= 0; i--) {
        ...
        if (!Objects.equals(logData.getUpstreamId(), retractingInfo.getUpstreamId())
          ...
          recordsForSplit.add(
              LogRecordKafkaWithRetractInfo.ofRetract(
                  r,
