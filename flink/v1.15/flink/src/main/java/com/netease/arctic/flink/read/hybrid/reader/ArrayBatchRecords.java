...
class ArrayBatchRecords<T> implements RecordsWithSplitIds<ArcticRecordWithOffset<T>> {
  ...
  private ArrayBatchRecords(
      @Nullable String splitId, @Nullable Pool.Recycler<T[]> recycler, @Nullable T[] records,
      int numberOfRecords, RecordPosition[] positions, Set<String> finishedSplits) {
    Preconditions.checkArgument(numberOfRecords >= 0, "numberOfRecords can't be negative");
  ...
  private void setRecordWithOffset() {
    ...
    Preconditions.checkArgument(offset.currentInsertFileOffset() >= 0 || offset.currentDeleteFileOffset() >= 0,
        "fileOffset can't be negative");
    Preconditions.checkArgument(offset.currentInsertRecordOffset() >= 0,
        "numberOfRecords can't be negative");
    Preconditions.checkArgument(offset.currentDeleteRecordOffset() >= 0,
        "numberOfRecords can't be negative");
    recordWithOffset.set(records[position], offset.currentInsertFileOffset(), offset.currentInsertRecordOffset(),
        offset.currentDeleteFileOffset(), offset.currentDeleteRecordOffset());
  }
   ...
   * @param recycler        Because {@link FileDataIterator} with {@link RowData}
   *                        returns an iterator of reused RowData object,
   *                        we need to clone RowData eagerly when constructing a batch of records.
   *                        We can use object pool to reuse the RowData array object
   *                        which can be expensive to create.
   *                        This recycler can be provided to recycle the array object back to pool
   *                        after read is exhausted.
   *                        If the {@link FileDataIterator} returns an iterator of non-reused objects,
